...
// 使用 BPF_TABLE 底层语法确保跨版本兼容
BPF_TABLE("lru_hash", unsigned int, struct storm_val, storm_counters, 10240);

// 敏感文件前缀 Map（用户态可动态更新）
// key=前缀索引(0-15), value=前缀字符串
struct prefix_entry { char data[64]; };
BPF_TABLE("array", int, struct prefix_entry, sensitive_prefixes, 16);
BPF_TABLE("array", int, unsigned int, sensitive_count, 1);  // value[0] = 实际前缀数量
...
static __always_inline int is_sensitive_file(const char *filename) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        struct prefix_entry *entry = sensitive_prefixes.lookup((u32 *)&i);
        if (!entry || entry->data[0] == '\0') continue;
        char *prefix = entry->data;
...
class RuntimeGuardian:
    ...
    def load_sensitive_prefixes(self):
        ...
        # 定义与 eBPF struct prefix_entry 匹配的 ctypes 结构
        class PrefixEntry(ctypes.Structure):
            _fields_ = [("data", ctypes.c_char * 64)]
        for i, prefix in enumerate(prefixes[:16]):
            key = ctypes.c_uint(i)
            val = PrefixEntry()
            val.data = prefix.encode('utf-8')[:63] + b'\x00'
            prefix_map[key] = val
